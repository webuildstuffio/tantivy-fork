...
/// Minimum number of query terms required to activate MaxScore instead of Block-Max WAND.
/// For queries with ≥ MAXSCORE_MIN_TERMS terms, MaxScore's term-centric batching outperforms
/// WAND's document-centric alternating advances (better cache locality + SIMD-friendly).
const MAXSCORE_MIN_TERMS: usize = 4;

/// Compute the MaxScore essential boundary.
///
/// Returns the smallest index `e` such that `scorers[e..]` are essential and
/// `scorers[0..e]` are non-essential. Non-essential terms have a combined max-score
/// sum strictly less than `threshold`, so any document containing only non-essential
/// terms cannot beat `threshold` and is safely pruned.
///
/// Requires `scorers` to be sorted by `max_score` **ascending** (lowest first).
/// Returns `scorers.len()` when all terms are non-essential (total max score < threshold).
fn maxscore_essential_boundary(scorers: &[TermScorerWithMaxScore<'_>], threshold: Score) -> usize {
    // Build a cumulative prefix sum (lowest max-scores first). The first index i where
    // prefix_sum_including_i >= threshold is the essential boundary: every term at i..n-1
    // is needed to potentially beat threshold.  Terms 0..i-1 (prefix sum < threshold)
    // are non-essential — their combined max score cannot alone beat threshold.
    let mut prefix_sum = 0.0f32;
    for (i, scorer) in scorers.iter().enumerate() {
        if prefix_sum + scorer.max_score >= threshold {
            return i;
        }
        prefix_sum += scorer.max_score;
    }
    scorers.len()
}
...
/// MaxScore inner loop.
///
/// `scorers` must be sorted by `max_score` **ascending** before calling.
///
...
fn maxscore_loop(
    ...
    loop {
        let essential_start = maxscore_essential_boundary(scorers, *threshold);

        if essential_start >= scorers.len() {
            break;
        }

        let pivot_doc = scorers[essential_start..]
            .iter()
            .map(|s| s.doc())
            .min()
        ...
        for scorer in scorers[essential_start..].iter_mut() {
            if scorer.doc() == pivot_doc {
                score += scorer.score();
            }
        }

        // Batch-seek non-essential terms to the candidate in one pass.
        // Skip scorers already past pivot_doc (seeking backwards panics).
        for scorer in scorers[..essential_start].iter_mut() {
            let curr = scorer.doc();
            if curr > pivot_doc {
                continue;
            }
            if curr == pivot_doc || scorer.seek(pivot_doc) == pivot_doc {
                score += scorer.score();
            }
        }
        ...
        // Advance every scorer sitting on pivot_doc.
...
/// Implements dynamic top-k pruning for multi-term BM25 queries.
///
/// Routes to MaxScore for queries with ≥ [`MAXSCORE_MIN_TERMS`] terms (term-centric, cache
/// friendly, SIMD-vectorizable), and falls back to Block-Max WAND for shorter queries.
///
/// Block-Max WAND reference: "Faster Top-k Document Retrieval Using Block-Max Indexes"
/// <http://engineering.nyu.edu/~suel/papers/bmw.pdf>
///
/// MaxScore reference: turbopuffer FTS v2 blog (Adrien Grand / Morgan Gallant, Dec 2025),
/// Lucene LUCENE-8135.
pub fn block_wand(
    ...
    if scorers.len() >= MAXSCORE_MIN_TERMS {
        scorers.sort_by(|a, b| {
            a.max_score
                .partial_cmp(&b.max_score)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        maxscore_loop(&mut scorers, &mut threshold, callback);
        return;
    }
...
mod tests {
    ...
    /// Run MaxScore directly (bypassing the scorers.len() < MAXSCORE_MIN_TERMS guard)
    /// and compare results against the brute-force union scanner.
    fn compute_checkpoints_maxscore(
        mut term_scorers: Vec<TermScorer>,
        n: usize,
    ) -> Vec<(DocId, Score)> {
        let mut heap: BinaryHeap<Float> = BinaryHeap::with_capacity(n);
...

        {
            ...
            let limit_ref = &mut limit;
            super::maxscore_loop(&mut scorers, limit_ref, &mut |doc, score| {
                heap_ref.push(Float(score));
                if heap_ref.len() > n {
                    heap_ref.pop().unwrap();
                }
                let new_limit = if heap_ref.len() == n {
                    heap_ref.peek().unwrap().0
                } else {
                    0.0
                };
                if !nearly_equals(score, new_limit) {
                    checkpoints_ref.push((doc, score));
                }
                new_limit
            });
        }
    ...
    fn test_maxscore_equiv_aux(posting_lists: &[Vec<(DocId, u32)>], fieldnorms: &[u32]) {
        const REPEAT: usize = 64;
        let fieldnorms_expanded = fieldnorms
            .iter()
            .cloned()
        ...
        for top_k in 1..4 {
            ...
            let checkpoints_manual =
                compute_checkpoints_manual(term_scorers.clone(), top_k, max_doc as u32);
            assert_eq!(
                checkpoints_maxscore.len(),
                checkpoints_manual.len(),
                "MaxScore and brute-force checkpoint count differ (top_k={top_k})"
            );
            for (&(l_doc, l_score), &(r_doc, r_score)) in
                checkpoints_maxscore.iter().zip(checkpoints_manual.iter())
            {
                assert_eq!(l_doc, r_doc, "doc mismatch top_k={top_k}");
                assert!(
                    nearly_equals(l_score, r_score),
                    "score mismatch top_k={top_k}: MaxScore={l_score} brute={r_score}"
                );
            }
        }
    }
    ...
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(500))]
        #[test]
        fn test_maxscore_equiv_two_scorers((posting_lists, fieldnorms) in gen_term_scorers(2)) {
            test_maxscore_equiv_aux(&posting_lists[..], &fieldnorms[..]);
        }
    }
    ...
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(500))]
        #[test]
        fn test_maxscore_equiv_four_scorers((posting_lists, fieldnorms) in gen_term_scorers(4)) {
            test_maxscore_equiv_aux(&posting_lists[..], &fieldnorms[..]);
        }
    }
    ...
    fn test_maxscore_single_term() {
        let posting_list = vec![(0u32, 2u32), (3, 5), (7, 1)];
        let fieldnorms = vec![10u32; 10];
